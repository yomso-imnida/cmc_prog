...
public:
    ...
    void save_pos(fpos_t &pos) const { 
        fgetpos(fp, &pos); 
    }
    // Восстанавливает позицию (прыгаем обратно)
    void restore_pos(const fpos_t &pos) { 
        fsetpos(fp, &pos); 
    }
};
...
void Parser::assignment_chain() {
    ...
    check_id_chains();          // curr_lex - первый id
    elems.push_back(c_val);
    gl();                       // Пропустили id
    if (c_type != LEX_ASSIGN)  // обязательно =
        throw curr_lex;
    gl();  // Пропустили =
    ...
    while (c_type == LEX_ID) {
        ...
        // Сохраняем состояние сканера и парсера
        fpos_t file_pos; // Позиция в файле
        scan.save_pos(file_pos);
        Lex savedLex = curr_lex;
        type_of_lex savedTyp = c_type;
        int savedVal = c_val;
        ...
        if (c_type == LEX_ASSIGN) {
            check_id_chains();
            elems.push_back(nextId);
            gl();  // Пропустили = и продолжаем
        }
        else {
            // НЕ цепочка - откатываем всё
            curr_lex = savedLex;
            c_type = savedTyp;
            c_val = savedVal;
            scan.restore_pos(file_pos);
            break;
        }
    }

    // Справа - всегда общее выражение
    E();

    // Генерируем ПОЛИЗ для всех elems
    for(int i = int(elems.size()) - 1; i >= 0; --i) {
        if (i != 0)
            poliz.push_back(Lex(LEX_DUP));
        ...
        while ( first || c_type == LEX_COMMA ) {
            ...
            if (c_type == LEX_ASSIGN) {
                ...
                if (var_type == LEX_STRING)
                    poliz.push_back(Lex(LEX_STR, c_val, 0, curr_lex.get_str()));
                else
                    poliz.push_back(Lex(c_type, c_val));
            
                poliz.push_back(Lex(POLIZ_ADDRESS, idx));
                poliz.push_back(Lex(LEX_ASSIGN));
                TID[idx].put_assign();

                gl(); // Пропустили константу
            }
        }
...
void Parser::S() {
    if (c_type == LEX_IF) {
        ...
        // Теперь текущая лексема должна быть ';'
        if (c_type != LEX_SEMICOLON)
            throw curr_lex;
        gl();  // Пропускаем ';'
    }
...
// Проходит по вектору poliz и выполняет программу
class Executer {
    std::vector<Lex> poliz;
    std::map<int, std::string> string_memory; // Строковые переменные по индексу
    // string x = "hello";  ->  в string_memory[5] хранится "hello" (индекс у x - 5)

public:
    explicit Executer(std::vector<Lex> p) : poliz(std::move(p)) {}

    void execute() {
        std::stack<Lex> st;  // Стек
        ...
        for (size_t i = 0; i < poliz.size(); ++i) {
            ...
            switch (lex.get_type()) {
                ...
                case LEX_WRITE: {
                    Lex val = st.top(); st.pop();
                    if (val.get_type() == LEX_STR) {
                        std::string s = val.get_str();
                        std::cout << '"' << s << "\"  ";
                    } else if (val.get_type() == LEX_ID) {
                        int idx = val.get_value();
                        if (TID[idx].get_type() == LEX_STRING)
                            std::cout << '"' << string_memory[idx] << "\"  ";
                        else
                            std::cout << TID[idx].get_value() << "  ";
                    } else {
                ...
                case LEX_ASSIGN: {
                    ...
                    if (TID[idx].get_type() == LEX_STRING) {
                        std::string val;
                        if (r.get_type() == LEX_ID && TID[r.get_value()].get_type() == LEX_STRING)
                            val = string_memory[r.get_value()];
                        else if (r.get_type() == LEX_STR)
                            val = r.get_str();
                        else {
                            std::string msg = "Неверный тип при присваивании строки: тип = " + std::to_string(r.get_type());
                            throw msg.c_str();
                        }
                        string_memory[idx] = val;
                    } else {
                        int val = (r.get_type() == LEX_ID) ? TID[r.get_value()].get_value() : r.get_value();
                        TID[idx].put_value(val);
                    }
                    TID[idx].put_assign();
                    break;
                }
                ...
                case LEX_DUP: {
                    st.push(st.top()); // Продублировали верх стека
                    break;
                }
                ...
                case LEX_READ: {
                    ...
                    if (TID[idx].get_type() == LEX_STRING) {
                        ...
                        string_memory[idx] = s;
                        TID[idx].put_assign();
                    } else if (TID[idx].get_type() == LEX_INT) {
                        int x;
                        std::cin >> x;
                        TID[idx].put_value(x);
                        TID[idx].put_assign();
                ...
                case POLIZ_FGO: {
                    Lex to = st.top(); st.pop();
                    Lex cond = st.top(); st.pop();
                    int val = (cond.get_type() == LEX_ID) ? TID[cond.get_value()].get_value() : cond.get_value();
                    if (!val) i = to.get_value() - 1;
                    break;
                }
                ...
                case LEX_PLUS: {
                    ...
                    bool l_is_str = (l.get_type() == LEX_STR) || (l.get_type() ==
                                    LEX_ID && TID[l.get_value()].get_type() == LEX_STRING);
                    bool r_is_str = (r.get_type() == LEX_STR) || (r.get_type() ==
                                    LEX_ID && TID[r.get_value()].get_type() == LEX_STRING);

                    if (l_is_str && r_is_str) {
                        std::string ls = (l.get_type() == LEX_STR) ? l.get_str() : string_memory[l.get_value()];
                        std::string rs = (r.get_type() == LEX_STR) ? r.get_str() : string_memory[r.get_value()];
                        std::string result = ls + rs;
                        static int dynamic_string_counter = 1000000; // Большой номер для временных строк
                        st.push(Lex(LEX_STR, dynamic_string_counter++, 0, result));
                    } else {
                        int lv = (l.get_type() == LEX_ID) ? TID[l.get_value()].get_value() : l.get_value();
                        int rv = (r.get_type() == LEX_ID) ? TID[r.get_value()].get_value() : r.get_value();
                        st.push(Lex(LEX_NUM, lv + rv));
                    }
                    break;
                }
                ...
                case LEX_OR: {
                    ...
                    switch (lex.get_type()) {
                        case LEX_MINUS: res = lv - rv; break;
                        case LEX_TIMES: res = lv * rv; break;
                        case LEX_SLASH:
                            if (rv == 0)
                                throw "Деление на 0";
                            res = lv / rv; break;
                        case LEX_MOD:
                            if (rv == 0)
                                throw "Остаток от деления на 0";
                            res = lv % rv; break;
                        case LEX_EQ:  res = (lv == rv); break;
                        case LEX_NEQ: res = (lv != rv); break;
                        case LEX_LSS: res = (lv < rv); break;
                        case LEX_GTR: res = (lv > rv); break;
                        case LEX_LEQ: res = (lv <= rv); break;
                        case LEX_GEQ: res = (lv >= rv); break;
                        case LEX_AND: res = lv && rv; break;
                        case LEX_OR:  res = lv || rv; break;
                        default: throw "Неизвестная операция";
                    }
                    st.push(Lex(LEX_NUM, res));
                    break;
                }

                default:
                    throw "Необработанная лексема при исполнении";
            }
        }
...
void print_input_file(const char* filename) {
    FILE* fp;

    if (!(fp = fopen(filename, "r")))
        throw "Файл не удалось открыть";

    std::cout << "--- Программа ---" << std::endl;
